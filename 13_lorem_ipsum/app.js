...
form.addEventListener("submit", e=>{
  ...
  // input태그에 사용자가 입력한 값(문자열)을 정수로 파싱
  const value = parseInt(amount.value);
  const random = Math.floor(Math.random() * text.length);
  console.log(random);

  // value가 숫자가 아니거나 음수이거나 8을 넘어가면
  if(isNaN(value) || value < 0 || value > 8){
    // 하나의 결과값만 출력
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  }else{
    // 선택된 value에 따라 임시 글 개수가 정해짐.
    // ( 화면에 출력할 text 배열의 요소 개수가 정해짐. )
    let tempText = text.slice(0, value);
    // 정해진 개수만큼 배열 요소를 모두 출력
    tempText = tempText.map(item=>{
      return `<p class="result">${item}</p>`;
    }).join(""); // 배열 요소를 하나의 문자열로 연결해서 저장

    console.log(tempText);
    result.innerHTML = tempText; // 화면에 출력
  }
